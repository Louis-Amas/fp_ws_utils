...
pub trait WsHandler<S> {
    fn handle<'a>(&'a self, ws: &'a mut WsStream, state: &'a mut S, msg: &'a Message) -> BoxFuture<'a, Result<HandlerOutcome>>;
}

impl<F, S> WsHandler<S> for F
where
    F: for<'a> Fn(&'a mut WsStream, &'a mut S, &'a Message) -> BoxFuture<'a, Result<HandlerOutcome>>,
{
    fn handle<'a>(&'a self, ws: &'a mut WsStream, state: &'a mut S, msg: &'a Message) -> BoxFuture<'a, Result<HandlerOutcome>> {
        self(ws, state, msg)
    }
...
impl<S> WsHandler<S> for HNil {
    fn handle<'a>(&'a self, _ws: &'a mut WsStream, _state: &'a mut S, _msg: &'a Message) -> BoxFuture<'a, Result<HandlerOutcome>> {
